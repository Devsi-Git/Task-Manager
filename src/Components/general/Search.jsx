    ...
    <>
      ...
      <AnimatePresence>
        {resultSearch.length > 0 ? (
          <motion.ul
            className="top-17 max-600:top-21 max-740:top-15 z-20 absolute flex flex-col gap-2 bg-[#f3f3f327] shadow-lg dark:shadow-[#ffffff57] backdrop-blur-lg max-600:mx-3 p-3 max-600:p-2 border-[#ffffff87] border-1 rounded-2xl w-fit overflow-hidden"
          ...
          >
            {resultSearch.map((res) => (
              <li
                className="bg-amber-200 dark:bg-[#535C91] p-2 rounded-xl ring-amber-300 hover:ring-2 dark:ring-[#1B1A55] min-w-20 600:min-w-30 dark:text-[#edebff] max-600:text-xs max-990:text-sm text-center text-nowrap transition-all cursor-pointer hover"
                key={res.id}
                onClick={() => {
                  scrollToCard(res.id);
                  setMenu(false);
